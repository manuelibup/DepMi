...
*   **Digital Products (Sessions 85, 118)**: `Order.isDigital Boolean @default(false)`. Checkout bypasses address/dispatch for digital orders. 48h escrow auto-release via `/api/cron/auto-release-digital` (hourly, `Bearer CRON_SECRET` — **register on cron-job.org**). Orders UI: digital orders show stepper `Placed → Paid → Available` (not the physical "Shipped" step) + "📖 Read Now" button linking to `/read/[orderId]`. ProductCard + product detail show "⚡ Instant Delivery" badge.
  - **Secure ebook reader** (`/read/[orderId]`): Auth-gated server component verifies buyer + status (CONFIRMED+). `EbookReader` client loads PDF.js 3.11.174 UMD via `next/script` CDN, renders to canvas with diagonal watermark (`@buyerUsername`), `user-select: none`, right-click + Ctrl+S/P blocked. Raw Cloudinary URL never reaches the browser — `/api/read/[orderId]` proxies bytes server-side.
  - **Download route** (`/api/download/[orderId]`): Same auth pattern, returns non-PDF digital files as `Content-Disposition: attachment`.
