...
export default function VerifyEmailPage() {
    ...
    if (status === 'unauthenticated' || !session?.user?.email) {
        // Not authenticated or missing email -> send to login
        router.push('/login?callbackUrl=/verify-email');
        return null;
    }
    ...
    const handleVerifyOTP = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Optionally update session if we modify KYC or emailVerified field
                await update();
        ...
        <main style={{ maxWidth: '440px', margin: '4rem auto', padding: '0 1.5rem', fontFamily: 'var(--font-heading)' }}>
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem', fontWeight: 700 }}>
                    {step === 1 ? 'Verify Your Email' : 'Enter Email Passcode'}
                </h1>
                <p style={{ color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                    {step === 1
                        ? 'Secure your DepMi account by verifying your email address.'
            ...
            <div style={{ background: 'var(--card-bg)', padding: '2rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)' }}>
                ...
                {step === 1 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        <div>
                            <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Current Email Address</p>
                            <div style={{
                                padding: '1rem',
